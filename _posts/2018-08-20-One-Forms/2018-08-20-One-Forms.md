...
We will see that the term $\Partial{\vec r}{u}$ is indeed a coordinate dependent basis vector. If we take the curve $\Gamma$ as $\begin{cases} u =u(t)\\ v =2 \end{cases}$ in the left figure (or $w=3$ in the right), $\Eqn{vectorToOperator}$ becomes
