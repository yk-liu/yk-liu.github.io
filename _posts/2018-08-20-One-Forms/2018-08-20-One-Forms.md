...
>
>    This particular basis $ \uvec{e}_{(\mu)}=\partial_\mu $ is known as a coordinate basis for $ T_{p} $; it is the formalization of the notion of setting up the basis vectors to point along the coordinate axes. There is no reason why we are limited to coordinate bases when we consider tangent vectors; it is sometimes more convenient, for example, to use orthonormal bases of some sort. However, the coordinate basis is very simple and natural, and we will use it almost exclusively throughout the course.
>
...
>
> 1. $c_1(0) = c_2(0) = p$
> 2. $ \left. \frac{\d x^µ(c_1(t))}{\d t} \right\rvert _{t=0} =\left.\frac{\d x^µ(c_2(t))}{\d t}\right\rvert_{t=0}$
>
