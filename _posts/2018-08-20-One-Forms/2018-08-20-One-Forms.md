...
\begin{align}
\vec X &= \D{\vec r(t)}{t} \notag \\
&=\left.\Partial{\vec r(u,v)}{u}\right|_{\substack{u=u_0\\v=v_0}} \left.\D{u(t)}{t}\right|_{t=t_0}+\left.\Partial{\vec r(u,v)}{v}\right|_{\substack{u=u_0\\v=v_0}}\left.\D{v(t)}{t}\right|_{t=t_0} \notag\\
&=\dot u(t_0)\ \left.\Partial{\vec r(u,v)}{u}\right|_{\substack{u=u_0\\v=v_0}} +\dot v(t_0)\ \left.\Partial{\vec r(u,v)}{v}\right|_{\substack{u=u_0\\v=v_0}}
...
\begin{align}
\vec X &= \D{\vec r(t)}{t} \notag \\
&=\left.\Partial{\vec r(u,v)}{u}\right|_{\substack{u=u_0\\v=v_0}} \left.\D{u(t)}{t}\right|_{t=t_0}+0 \notag\\
&=\dot u (t_0)\ \left.\Partial{\vec r(u,v)}{u}\right|_{\substack{u=u_0\\v=v_0}} 
\end{align}
$$

notice for example,  $ \Gamma=\left{\substack{u=k\cdot t\\ v=2}\right. $  This means if we take a parametrized curve along v=2, the tangent vector at point p will always be some number times $\left.\Partial{\vec r}{u}\right|_{\substack{u=u_0\\v=v_0}}$ .  In other words, ** [the partial differential operator *IS* a unit vector] **.

Reiterating, a tangent vector is  $ \Gamma=\left{\substack{u=k\cdot t\\ v=2}\right.$
...
>
> 1. $c_1(0) = c_2(0) = p$
> 2.  $ \left. \frac{\d x^µ(c_1(t))}{\d t} \right| _{t=0} =\left.\frac{\d x^µ(c_2(t))}{\d t}\right|_{t=0} $
>
...
> $[c(t)] = \left\{ \tilde{c}(t) \mid \tilde{c}(0)=c(0) \text{ and } \left.\frac{\d x^\mu (\tilde{c}(t))}{\d t} \right|_{t=0} = \left.\frac{\d x^\mu (c(t))}{\d t} \right|_{t=0} \right\}, $
>
