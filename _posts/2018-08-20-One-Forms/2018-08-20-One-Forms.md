...
2. To find the basis of this vector space. 
   ...
   $$
   \form f = \Partial{f}{x^i} \uvec {\bf e}
   $$

   where $\uvec e$ is a hungry operator on vectors. Now the job is to find the exact expression of $\uvec e$. In $\Eqn{basis-of-oneform}$, the basis of one-form was introduced to cancel out the effect of operator $\Partial{}{x^i}$. Putting the context of vectors and covectors aside, just to balance the equation appeared in $\Eqn{basis-of-oneform}$ the product of operator $\uvec e$ and operator $\Partial{}{x^i}​$ in the most conventional sense should be the identity operator.
