...
Now consider the set of linear functions defined on $V$ (vectors) that have values in $K$ (numbers), i.e. $f:V\rightarrow K$. Such linear functions on vector spaces are called **linear functionals**. They apparently form a vector space, since: 
...
> - If the original space is finite dimensional, vectors can be sent to covectors through a isomorphism (in coordinates, via transpose). For example, Dirac-bras ( $ \bra{v} $ ) and -kets (( $ \ket{v} $ )) are dual vectors.
> - The isomorphism mentioned in the previous item can be written as a definition of map 
>
> $$
...
Here we construct the **covector space** of $V$ and map between vectors and covectors:
...
4. Define $\d x_i (\uvec x_j)\dfdas \delta_{i,j}$. Other choices can always seen as a (unitary) transformation of this simple definition.

5. **[map between vectors and covectors]** A covector $\dual {\vec v}$  of vector $\vec v$ can be written in components $\dual{\vec v}=\dual v^\mu\d x_\mu$, conversely, a vector $\vec{\dual v}$ of a covector $\form v$, $\vec{\dual v}=\dual v^\mu \uvec x_\mu$ by directly interchanging $\d x_\mu$ between $\uvec x_\mu$.

6. A covector $\dual {\vec v}(\vec v)=\dual v^i \d x_i (v^i\uvec x_i)=\dual v^i v_i=\vec{\dual v }\cdot\vec v\in \R$

> ***IMPORTANT EXAMPLE:***
>
> The gradient and total derivative are dual vectors to each other. That's why above I made such a weird choice of name for covector basis. 
>
> $$
> \begin{align}
