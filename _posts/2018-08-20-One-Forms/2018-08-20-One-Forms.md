...
Though on a manifold, **vector** is defined to be a **tangent vector** to a curve in $\mani M$. Note the existence of a vector is independent of the coordinate or function. Like shown below, the "Big red arrow" $X$ tangent to the manifold depend solely on the curve and the point, not any local coordinate.
